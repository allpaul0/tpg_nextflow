/**
 * File generated with GEGELATI v2.0.0
 * On the 2025-11-24 14:17:20
 * With the CodeGen::TPGGenerationEngine.
 */

#include "TPG.h"

int bestProgram(double *results, int nb) {
	int bestProgram = 0;
	double bestScore = (isnan(results[0]))? -INFINITY : results[0];
	for (int i = 1; i < nb; i++) {
		double challengerScore = (isnan(results[i]))? -INFINITY : results[i];
		if (challengerScore >= bestScore) {
			bestProgram = i;
			bestScore = challengerScore;
		}
	}
	return bestProgram;
}

enum vertices {T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, A11, A12, A13, A14, A15, A16, A17, A18, };

void inferenceTPG(double* actions) {

	enum vertices currentVertex = T10;
	while(1) {
		switch (currentVertex) {
		case T0: {
				const enum vertices next[2] = { A11, A12,  };

				double T0Scores[2];

				T0Scores[0] = P0();
				T0Scores[1] = P1();

				int best = bestProgram(T0Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T1: {
				const enum vertices next[2] = { A12, A14,  };

				double T1Scores[2];

				T1Scores[0] = P2();
				T1Scores[1] = P3();

				int best = bestProgram(T1Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T2: {
				const enum vertices next[3] = { A13, A12, T0,  };

				double T2Scores[3];

				T2Scores[0] = P4();
				T2Scores[1] = P5();
				T2Scores[2] = P6();

				int best = bestProgram(T2Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T3: {
				const enum vertices next[4] = { A15, A11, T1, T1,  };

				double T3Scores[4];

				T3Scores[0] = P7();
				T3Scores[1] = P8();
				T3Scores[2] = P9();
				T3Scores[3] = P10();

				int best = bestProgram(T3Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T4: {
				const enum vertices next[3] = { T2, A16, T2,  };

				double T4Scores[3];

				T4Scores[0] = P11();
				T4Scores[1] = P12();
				T4Scores[2] = P13();

				int best = bestProgram(T4Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T5: {
				const enum vertices next[7] = { T2, T1, A13, T1, A15, A11, T1,  };

				double T5Scores[7];

				T5Scores[0] = P14();
				T5Scores[1] = P9();
				T5Scores[2] = P15();
				T5Scores[3] = P10();
				T5Scores[4] = P16();
				T5Scores[5] = P8();
				T5Scores[6] = P17();

				int best = bestProgram(T5Scores, 7);
				currentVertex = next[best];
				break;
			}
		case T6: {
				const enum vertices next[6] = { T4, A17, A13, A11, A13, A12,  };

				double T6Scores[6];

				T6Scores[0] = P18();
				T6Scores[1] = P19();
				T6Scores[2] = P20();
				T6Scores[3] = P21();
				T6Scores[4] = P22();
				T6Scores[5] = P23();

				int best = bestProgram(T6Scores, 6);
				currentVertex = next[best];
				break;
			}
		case T7: {
				const enum vertices next[3] = { T1, A17, A13,  };

				double T7Scores[3];

				T7Scores[0] = P10();
				T7Scores[1] = P24();
				T7Scores[2] = P25();

				int best = bestProgram(T7Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T8: {
				const enum vertices next[5] = { A12, T1, T6, T7, A11,  };

				double T8Scores[5];

				T8Scores[0] = P26();
				T8Scores[1] = P27();
				T8Scores[2] = P28();
				T8Scores[3] = P29();
				T8Scores[4] = P30();

				int best = bestProgram(T8Scores, 5);
				currentVertex = next[best];
				break;
			}
		case T9: {
				const enum vertices next[3] = { T3, T5, T8,  };

				double T9Scores[3];

				T9Scores[0] = P31();
				T9Scores[1] = P32();
				T9Scores[2] = P33();

				int best = bestProgram(T9Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T10: {
				const enum vertices next[3] = { A18, T1, T9,  };

				double T10Scores[3];

				T10Scores[0] = P34();
				T10Scores[1] = P35();
				T10Scores[2] = P36();

				int best = bestProgram(T10Scores, 3);
				currentVertex = next[best];
				break;
			}
		case A11: {
				actions[0] = 4;
				return;
			}
		case A12: {
				actions[0] = 6;
				return;
			}
		case A13: {
				actions[0] = 5;
				return;
			}
		case A14: {
				actions[0] = 2;
				return;
			}
		case A15: {
				actions[0] = 7;
				return;
			}
		case A16: {
				actions[0] = 8;
				return;
			}
		case A17: {
				actions[0] = 0;
				return;
			}
		case A18: {
				actions[0] = 1;
				return;
			}
		}
	}
}
