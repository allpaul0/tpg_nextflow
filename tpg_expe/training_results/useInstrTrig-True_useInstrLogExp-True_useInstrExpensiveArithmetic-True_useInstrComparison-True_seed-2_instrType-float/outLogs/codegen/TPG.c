/**
 * File generated with GEGELATI v2.0.0
 * On the 2025-11-24 14:17:22
 * With the CodeGen::TPGGenerationEngine.
 */

#include "TPG.h"

int bestProgram(float *results, int nb) {
	int bestProgram = 0;
	float bestScore = (isnan(results[0]))? -INFINITY : results[0];
	for (int i = 1; i < nb; i++) {
		float challengerScore = (isnan(results[i]))? -INFINITY : results[i];
		if (challengerScore >= bestScore) {
			bestProgram = i;
			bestScore = challengerScore;
		}
	}
	return bestProgram;
}

enum vertices {T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, A11, A12, A13, A14, A15, A16, A17, };

void inferenceTPG(float* actions) {

	enum vertices currentVertex = T10;
	while(1) {
		switch (currentVertex) {
		case T0: {
				const enum vertices next[4] = { A11, A12, A13, A14,  };

				float T0Scores[4];

				T0Scores[0] = P0();
				T0Scores[1] = P1();
				T0Scores[2] = P2();
				T0Scores[3] = P3();

				int best = bestProgram(T0Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T1: {
				const enum vertices next[3] = { A12, A13, A14,  };

				float T1Scores[3];

				T1Scores[0] = P4();
				T1Scores[1] = P5();
				T1Scores[2] = P6();

				int best = bestProgram(T1Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T2: {
				const enum vertices next[3] = { A15, A11, A12,  };

				float T2Scores[3];

				T2Scores[0] = P7();
				T2Scores[1] = P8();
				T2Scores[2] = P9();

				int best = bestProgram(T2Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T3: {
				const enum vertices next[5] = { A12, A16, A13, T1, A11,  };

				float T3Scores[5];

				T3Scores[0] = P10();
				T3Scores[1] = P11();
				T3Scores[2] = P12();
				T3Scores[3] = P13();
				T3Scores[4] = P0();

				int best = bestProgram(T3Scores, 5);
				currentVertex = next[best];
				break;
			}
		case T4: {
				const enum vertices next[1] = { T3,  };

				float T4Scores[1];

				T4Scores[0] = P14();

				int best = bestProgram(T4Scores, 1);
				currentVertex = next[best];
				break;
			}
		case T5: {
				const enum vertices next[2] = { A16, T4,  };

				float T5Scores[2];

				T5Scores[0] = P15();
				T5Scores[1] = P16();

				int best = bestProgram(T5Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T6: {
				const enum vertices next[2] = { T4, A17,  };

				float T6Scores[2];

				T6Scores[0] = P17();
				T6Scores[1] = P18();

				int best = bestProgram(T6Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T7: {
				const enum vertices next[1] = { T4,  };

				float T7Scores[1];

				T7Scores[0] = P19();

				int best = bestProgram(T7Scores, 1);
				currentVertex = next[best];
				break;
			}
		case T8: {
				const enum vertices next[2] = { T0, A13,  };

				float T8Scores[2];

				T8Scores[0] = P20();
				T8Scores[1] = P21();

				int best = bestProgram(T8Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T9: {
				const enum vertices next[6] = { T7, T0, A13, T5, T6, T2,  };

				float T9Scores[6];

				T9Scores[0] = P22();
				T9Scores[1] = P23();
				T9Scores[2] = P24();
				T9Scores[3] = P25();
				T9Scores[4] = P26();
				T9Scores[5] = P27();

				int best = bestProgram(T9Scores, 6);
				currentVertex = next[best];
				break;
			}
		case T10: {
				const enum vertices next[4] = { A14, T8, T9, A17,  };

				float T10Scores[4];

				T10Scores[0] = P28();
				T10Scores[1] = P29();
				T10Scores[2] = P30();
				T10Scores[3] = P31();

				int best = bestProgram(T10Scores, 4);
				currentVertex = next[best];
				break;
			}
		case A11: {
				actions[0] = 4;
				return;
			}
		case A12: {
				actions[0] = 7;
				return;
			}
		case A13: {
				actions[0] = 5;
				return;
			}
		case A14: {
				actions[0] = 0;
				return;
			}
		case A15: {
				actions[0] = 6;
				return;
			}
		case A16: {
				actions[0] = 1;
				return;
			}
		case A17: {
				actions[0] = 2;
				return;
			}
		}
	}
}
