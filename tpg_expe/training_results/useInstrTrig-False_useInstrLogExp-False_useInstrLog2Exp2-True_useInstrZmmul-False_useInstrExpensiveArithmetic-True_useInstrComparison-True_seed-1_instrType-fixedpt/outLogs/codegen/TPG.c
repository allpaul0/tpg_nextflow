/**
 * File generated with GEGELATI v2.0.0
 * On the 2025-11-24 14:17:04
 * With the CodeGen::TPGGenerationEngine.
 */

#include "TPG.h"

int bestProgram(fixedpt *results, int nb) {
	int bestProgram = 0;
	fixedpt bestScore = results[0];
	for (int i = 1; i < nb; i++) {
		fixedpt challengerScore = results[i];
		if (challengerScore >= bestScore) {
			bestProgram = i;
			bestScore = challengerScore;
		}
	}
	return bestProgram;
}

enum vertices {T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, A14, A15, A16, A17, A18, A19, A20, };

void inferenceTPG(fixedpt* actions) {

	enum vertices currentVertex = T13;
	while(1) {
		switch (currentVertex) {
		case T0: {
				const enum vertices next[3] = { A18, A16, A15,  };

				fixedpt T0Scores[3];

				T0Scores[0] = P0();
				T0Scores[1] = P1();
				T0Scores[2] = P2();

				int best = bestProgram(T0Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T1: {
				const enum vertices next[4] = { A14, A15, A17, T0,  };

				fixedpt T1Scores[4];

				T1Scores[0] = P3();
				T1Scores[1] = P4();
				T1Scores[2] = P5();
				T1Scores[3] = P6();

				int best = bestProgram(T1Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T2: {
				const enum vertices next[4] = { A14, A19, A15, A19,  };

				fixedpt T2Scores[4];

				T2Scores[0] = P7();
				T2Scores[1] = P8();
				T2Scores[2] = P9();
				T2Scores[3] = P10();

				int best = bestProgram(T2Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T3: {
				const enum vertices next[4] = { A14, T0, A19, A15,  };

				fixedpt T3Scores[4];

				T3Scores[0] = P11();
				T3Scores[1] = P12();
				T3Scores[2] = P10();
				T3Scores[3] = P9();

				int best = bestProgram(T3Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T4: {
				const enum vertices next[4] = { T0, A17, T1, A19,  };

				fixedpt T4Scores[4];

				T4Scores[0] = P12();
				T4Scores[1] = P13();
				T4Scores[2] = P14();
				T4Scores[3] = P10();

				int best = bestProgram(T4Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T5: {
				const enum vertices next[2] = { T4, T2,  };

				fixedpt T5Scores[2];

				T5Scores[0] = P15();
				T5Scores[1] = P16();

				int best = bestProgram(T5Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T6: {
				const enum vertices next[2] = { T3, T5,  };

				fixedpt T6Scores[2];

				T6Scores[0] = P17();
				T6Scores[1] = P18();

				int best = bestProgram(T6Scores, 2);
				currentVertex = next[best];
				break;
			}
		case T7: {
				const enum vertices next[1] = { T5,  };

				fixedpt T7Scores[1];

				T7Scores[0] = P19();

				int best = bestProgram(T7Scores, 1);
				currentVertex = next[best];
				break;
			}
		case T8: {
				const enum vertices next[5] = { T3, T5, T2, T3, T7,  };

				fixedpt T8Scores[5];

				T8Scores[0] = P20();
				T8Scores[1] = P21();
				T8Scores[2] = P22();
				T8Scores[3] = P23();
				T8Scores[4] = P24();

				int best = bestProgram(T8Scores, 5);
				currentVertex = next[best];
				break;
			}
		case T9: {
				const enum vertices next[3] = { T3, T6, T8,  };

				fixedpt T9Scores[3];

				T9Scores[0] = P25();
				T9Scores[1] = P26();
				T9Scores[2] = P27();

				int best = bestProgram(T9Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T10: {
				const enum vertices next[3] = { T9, T6, T8,  };

				fixedpt T10Scores[3];

				T10Scores[0] = P28();
				T10Scores[1] = P29();
				T10Scores[2] = P27();

				int best = bestProgram(T10Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T11: {
				const enum vertices next[4] = { T10, T9, T8, T3,  };

				fixedpt T11Scores[4];

				T11Scores[0] = P30();
				T11Scores[1] = P31();
				T11Scores[2] = P32();
				T11Scores[3] = P25();

				int best = bestProgram(T11Scores, 4);
				currentVertex = next[best];
				break;
			}
		case T12: {
				const enum vertices next[3] = { T6, T11, A20,  };

				fixedpt T12Scores[3];

				T12Scores[0] = P33();
				T12Scores[1] = P34();
				T12Scores[2] = P35();

				int best = bestProgram(T12Scores, 3);
				currentVertex = next[best];
				break;
			}
		case T13: {
				const enum vertices next[4] = { T8, T12, T9, T10,  };

				fixedpt T13Scores[4];

				T13Scores[0] = P36();
				T13Scores[1] = P37();
				T13Scores[2] = P38();
				T13Scores[3] = P39();

				int best = bestProgram(T13Scores, 4);
				currentVertex = next[best];
				break;
			}
		case A14: {
				actions[0] = 5;
				return;
			}
		case A15: {
				actions[0] = 6;
				return;
			}
		case A16: {
				actions[0] = 4;
				return;
			}
		case A17: {
				actions[0] = 0;
				return;
			}
		case A18: {
				actions[0] = 3;
				return;
			}
		case A19: {
				actions[0] = 8;
				return;
			}
		case A20: {
				actions[0] = 2;
				return;
			}
		}
	}
}
